...
import { GET_PROVIDERS_PAYMENTS, UPDATE_PROVIDER_PAYMENT } from './queries.gql'
...
const PaymentList = () => {
  ...
  const [updateProviderPayment] = useMutation(UPDATE_PROVIDER_PAYMENT, {
    refetchQueries: [GET_PROVIDERS_PAYMENTS],
  })
  const handleStatePayment = async (id: string, status: string) => {
                      ...
                      {user.isAdmin ? (
                        <>
                          <span
                            ...
                            onClick={() =>
                              handleStatePayment(payment.id, 'rejected')
                            }
                          >
                            Rechazar
                          </span>
                          <button
                            className="w-28 rounded bg-green-500 px-3 py-1 text-white hover:bg-green-600"
                            onClick={() =>
                              handleStatePayment(payment.id, 'pending')
                            }
                          >
                            Aceptar
                          </button>
