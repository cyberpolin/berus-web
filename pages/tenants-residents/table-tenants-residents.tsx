...
import { GET_RESIDENTS_TENANTS } from '../dashboard/queries.gql'
import Layout from '@/components/layout/NLayout'
import { useRouter } from 'next/router'
import UseAuth from '@/lib/UseAuth'
import { UPDATE_RESIDENT, UPDATE_TENANT } from '../dashboard/queries.gql'
...
const TableTenantsResidents = () => {
  ...
  const tenant = user.tenant?.properties?.[0]
  let ownerID
  if (resident?.id) {
    ownerID = resident?.theOwner?.user?.id
  } else {
    ownerID = user.id
  }
  ...
  const { loading, error, data } = useQuery(GET_RESIDENTS_TENANTS, {
    variables: {
      id: ownerID,
    },
  })
  const people = data?.user?.properties[0]
      ...
      <div className="mx-auto flex w-full max-w-[1400px] flex-col px-4 ">
        ...
        {!resident?.id && (
          <button
            ...
            onClick={() =>
              router.push(`/tenants-residents/add-tenant-resident`)
            }
          >
            {' '}
            Agregar +
