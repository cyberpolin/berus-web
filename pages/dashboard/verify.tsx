...
const Verify = () => {
  ...
  const [veryfyPhone, { data, error, loading, called }] = useLazyQuery(VERIFY_PHONE, {
    onCompleted: (data) =>
      data?.verifyPhone ? setTimeout(() => window?.location?.reload(), 10000) : null,
  });
  ...
  return (
    <Layout>
      <div className="w-full">
        <h1>
          Te enviamos un codigo por whatsapp para confirmar tu numero, por favor ingresa
          los 6 digitos.
        </h1>
        ...
        >
          {values.map((v, i) => (
            <input
              ...
              onFocus={() => clean(i)}
              className="center m-2 block w-1/2 rounded-lg border-2 border-gray-300 bg-gray-50 p-3 text-gray-900 sm:text-xl dark:border-gray-400 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400"
            />
          ))}
        </form>
